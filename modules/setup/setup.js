          ...
          return new Promise((resolve, reject) => {
            docker.pull(imageTag, (dockerPullError, dockerPullStream) => {
              if (dockerPullError) {
                ...
                console.log(
                  "setting up tezos node, this could take a while...."
                );
                ...
                docker.modem.followProgress(
                  dockerPullStream,
                  (__dockerModemError, __dockerModemOutput) => {
                    clearInterval(progress);
                    progressbar.update(100);
                    console.log(
                      this.helper.output(
    ...
    childprocess.exec(
      ...
      (error, __stdout, __stderr) => {
        if (__stdout === `${imageTag}:2.75GB\n`) {
          childprocess.exec(
            `docker ps -a -q  --filter ancestor=${imageTag} --format "{{.Image}}:{{.Names}}"`,
            (error, __stdout, __stderr) => {
              if (__stdout.includes(`${imageTag}:${containerName}\n`)) {
                console.log(
                  this.helper.outputInfo("Nodes are already running....")
                );
              } else {
                const _cliProgress = require("cli-progress");
                var self = this;
                ...
                setTimeout(() => {
                  const myInterval = setInterval(() => {
                    ...
                    request(
                      "http://localhost:18731/chains/main/blocks/head/protocols",
                      function(error, response, body) {
                        ...
                        if (data.protocol.startsWith("PsBaby")) {
                          ...
                          console.log(
                            self.helper.output(
                              "Nodes have been started successfully...."
                            )
                          );
                ...
                return new Promise((resolve, reject) => {
                  docker.createContainer(
                    createOptions,
                    function(createContainerErr, container) {
      ...
      (error, __stdout, __stderr) => {
        if (__stdout.includes(`${imageTag}\n`)) {
          ...
          docker.listContainers(function(err, containers) {
            ...
            console.log(
              self.helper.outputInfo(
                "Nodes have been stopped. Run 'tezster start-nodes' to restart."
              )
            );
          });
        } else console.log(this.helper.outputError("No Nodes are running...."));
      }
    );
  }
}
